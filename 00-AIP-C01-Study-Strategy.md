...
Recommended build order: 01 → 02 → 06 → 04 → 03 → 08 → 07 → 05. This front-loads the two biggest Domain 1 guides plus Guardrails/Responsible AI (Domain 3), which together cover the highest-yield material.

**Status: all 8 guides are built.** Because the build order diverges from the strategy numbering, the on-disk file numbers do not match the strategy guide numbers. Mapping (strategy guide → file):

| Strategy Guide | Topic | File on disk |
|---|---|---|
...
Each guide follows the same format as the ANS DX deep-dive: textbook-depth prose, Mermaid diagrams, comparison tables, an Exam-Relevant Distinctions checklist, and collapsible Knowledge Check quizzes, with a consolidated references section at the end.
...
```
Need current/private facts, low cost, fast iteration? -> RAG (Knowledge Bases)
Need domain tone/format/behavior baked in?           -> Fine-tune (SageMaker + LoRA)
Need to steer behavior with no infra change?          -> Prompt engineering + Guardrails
```

### Defense-in-Depth for Safe GenAI

```
Input:  Comprehend/Guardrails filter  ->  prompt-injection detection
Model:  Bedrock Guardrails (denied topics, grounding)
Output: Lambda post-processing  ->  JSON Schema validation  ->  PII redaction
```

### Cost Lever Priority (cheapest win first)

```
1. Token efficiency (prompt compression, context pruning)
2. Caching (prompt caching, semantic caching)
3. Model tiering / cascading (small model first, escalate)
4. Provisioned Throughput / batching (only at steady high volume)
```
