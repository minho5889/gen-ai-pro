    ...
    def retrieve(self, **kw):
        ...
        return {"retrievalResults": [
            {"content": {"text": "Prompt caching cheapens repeated prefixes."},
             "metadata": {"breadcrumb": "Guide 7 >> Prompt Caching"}, "score": 0.91},
            {"content": {"text": "Semantic caching skips the model call."},
             "metadata": {"breadcrumb": "Cram sheet D4"}, "score": 0.84},
        ]}
...
stub.client = lambda name, **kw: {"bedrock-agent-runtime": _AgentRT(),
                                  "bedrock-runtime": _BedrockRT()}[name]
...
try:
    urllib.request.urlopen(urllib.request.Request(
        f"{base}/chat", data=b"{}", headers={"Content-Type": "application/json"}), timeout=5)
...
req = urllib.request.Request(
    f"{base}/chat",
    data=json.dumps({"message": "caching?", "history": [
        {"role": "user", "text": "hi"}, {"role": "assistant", "text": "hello"}]}).encode(),
...
check("event order meta->tokens->done",
      kinds[0] == "meta" and kinds[-1] == "done" and kinds.count("token") == 2)
check("meta carries 2 breadcrumbed sources",
