...
**Diagram (described):** This diagram traces a request top to bottom through the three defense-in-depth layers. User input enters Layer 1, the input-filtering layer, where it flows through three steps in sequence: Comprehend performs PII detection, then Guardrails evaluates the input, then Lambda sanitizes it. The cleaned request passes into Layer 2, the model-controls layer, where a Bedrock Guardrails policy applies denied topics and grounding checks. From there the request reaches the Foundation model, which generates a response. That response enters Layer 3, the output-verification layer, where it flows through three steps: Lambda post-processing, then JSON Schema validation, then PII redaction. Finally the verified response is returned to the user. The flow is strictly sequential — input layer, model layer, model, output layer, user response.
...
**A:** B — Input filtering is structurally blind to anything that happens during or after generation. It cannot see a hallucinated fact, a toxic phrasing the model invents, or PII that enters through a retrieved chunk rather than the prompt. Those are properties of the output, so only output verification can catch them. A is the defense-in-depth trap; C and D describe unrelated concerns.
...
**Diagram (described):** This diagram shows the shape of the whole Guardrails service. User input flows into a guardrail input-evaluation step, then into the Foundation model, which produces output that flows into a guardrail output-evaluation step, and finally to the user response. Off to the side sits a box of the six guardrail policies — content filters, denied topics, word filters, sensitive-information (PII) filter, contextual grounding, and automated reasoning. Dashed "evaluates" arrows run from that policy box to both the input-evaluation step and the output-evaluation step, indicating that the same set of policies feeds the evaluation engine in both directions, wrapping the model on both sides.
...
- Denied topics: up to 30 *(ceiling point-in-time — verify)*, each a name plus definition (200 chars Classic / 1,000 Standard) plus up to 5 sample phrases.
...
**A:** Grounding and relevance, each with a threshold from 0 to 0.99. A response that invents information not present in the source fails the grounding check (it may still be relevant — on topic — but it is not supported by the source). A response that is correct but answers the wrong question fails the relevance check instead.
...
**Diagram (described):** This diagram shows a full moderation pipeline orchestrated by Step Functions. User input first hits AWS WAF at the perimeter, then enters the Step Functions orchestration. Inside that orchestration the steps run in sequence: Comprehend screens for PII and toxicity, then Guardrails evaluates the input, then Lambda validates the input, then the pipeline retrieves Knowledge Base chunks, then a screening step inspects those retrieved chunks, then the model is invoked, then Lambda post-processes the output, then JSON Schema validation runs, then Guardrails evaluates the output, and finally a decision diamond asks whether the content is flagged. If flagged ("yes"), the flow branches to a human-review step; if not flagged ("no"), the response is released to the user. The key teaching points the diagram embeds are that retrieved chunks get a dedicated screening step (the gap a guardrail alone misses) and that a branch sends flagged content to a human rather than straight to the user.
...
**A:** Structured outputs and citations are mutually exclusive for Anthropic models — requesting both in the same call returns a 400 error. The developer must choose one: keep citations and parse the response manually, or drop citations and use the JSON Schema structured-output format (via `outputConfig.textFormat` in Converse) to enforce the shape. Separately, a 400 can also occur if the supplied schema uses a JSON Schema feature outside the supported Draft 2020-12 subset, but the citations-plus-structured-output combination is the classic conflict here.
...
**Diagram (described):** This diagram shows the data-protection flow from source to model. S3 source data feeds into a "data-layer controls" group, where three controls run in sequence: Macie discovers and classifies the sensitive data, then Comprehend redacts the PII, then KMS encrypts the data at rest. The cleaned, encrypted output flows into the vector store, then into a retrieval step, then to the Guardrails control at the model boundary, and finally into the foundation model. The teaching point the diagram embeds is that the three data-layer controls all act behind the boundary, before data reaches the vector store and the model — so sensitive data is discovered, redacted, and encrypted upstream rather than relying on the guardrail at the boundary to catch it.

### AWS Lake Formation: Granular Access to the Corpus Data Lake

AWS Lake Formation is the data-layer control for the corpus itself when that corpus lives in a data lake. IAM alone grants access at the level of buckets, prefixes, and API actions; Lake Formation layers database-, table-, column-, and row-level permissions over data cataloged in the AWS Glue Data Catalog, with tag-based access control (LF-TBAC) for administering permissions at scale. In a GenAI context that granularity matters twice. First, upstream of ingestion: the pipeline role that syncs a Knowledge Base or assembles fine-tuning data should be able to read exactly the tables and columns approved for the model — excluding, say, a PII column — rather than everything in the bucket, so sensitive fields never enter the vector store or the training set at all. Second, for auditability: Lake Formation permissions are centrally administered and queryable, which gives a governance review one place to answer "what data could this model's pipeline actually read?" The exam cue is the phrase granular data access for FM data sources — Task 3.2 names Lake Formation explicitly: bucket- or prefix-level isolation is IAM and S3 policy; table-, column-, or row-level control over cataloged lake data is Lake Formation. The boundary framing from the start of this section still applies — Lake Formation is a data-layer control that shrinks what can ever reach the model; it does nothing at the invocation boundary.
...
| "Granular (table/column/row-level) access to the corpus data lake" | AWS Lake Formation over the Glue Data Catalog | IAM/S3 stop at bucket/prefix level; LF-TBAC administers at scale; data-layer control, never the invocation boundary |
...
**Diagram (described):** This diagram shows the access-control and connectivity path left to right. A workforce or end user authenticates through an external identity provider or Amazon Cognito, which maps them to a least-privilege IAM role. That role is assumed by the application running in a private subnet. The application reaches Bedrock through a PrivateLink endpoint, which connects to the Bedrock runtime. The application also assumes an agent service role, which likewise reaches the Bedrock runtime. Separately, Secrets Manager supplies credentials to a third-party Knowledge Base connector, and the agent service role connects to that same connector. The diagram's teaching points are that identity flows through an IdP to a scoped role, that traffic to Bedrock goes through a private endpoint rather than the public internet, and that third-party connector credentials come from Secrets Manager rather than being hardcoded.
...
**A:** B — The Converse and ConverseStream APIs do not have their own IAM actions. They ride on `bedrock:InvokeModel` and `bedrock:InvokeModelWithResponseStream` respectively, so the policy must grant InvokeModel (scoped to the Claude model ARN). There is no `bedrock:Converse` action to grant, which is why the policy authorizes nothing. C is wrong because invocation is a `bedrock-runtime` data-plane operation, not control plane, and D is wrong because base-model access is governed by identity-based policies, not resource policies.
...
**Diagram (described):** This diagram maps responsible-AI tools to the dimensions they operationalize. On one side sits a group of five dimensions: fairness, explainability, transparency, veracity and robustness, and controllability. Arrows connect each tool to the dimensions it serves. SageMaker Clarify points to both fairness and explainability. Model and Service Cards point to transparency. Watermark detection also points to transparency. The Bedrock agent trace points to both explainability and controllability. Guardrails and automated reasoning point to veracity and robustness. The teaching point the diagram embeds is that each responsible-AI dimension is backed by a concrete tool, and some tools (like Clarify and the agent trace) serve more than one dimension at once.
...
**A:** False. AWS AI Service Cards are authored by AWS, not by customers — you read them, you do not create them. The Nova family already has an AWS-published service card. The document you create to record your *own* model's intended use, limitations, risk rating, and evaluation results is a SageMaker Model Card. The memory aid: AI Service Card means AWS documents AWS's service; Model Card means you document your model.

**Q4:** A SageMaker Model Card has a risk-rating field. What are its allowed values?

**A:** unknown, low, medium, and high. The risk rating is a small enumerated field on the model card, alongside the intended uses (including where the model is not recommended), training details, evaluation results, observations, and recommendations. Model cards also carry an immutable version history — any edit other than an approval-status change spawns a new version — integrate with the SageMaker Model Registry, auto-populate metrics from Clarify and Model Monitor, and export to PDF.
...
**Diagram (described):** This diagram shows three governance pipelines feeding three downstream tools, plus a data-lineage path. First, Bedrock inference flows into Model Invocation Logging, which writes log bodies to Amazon S3, which is then queried by Athena for content analysis. Second, control-plane API calls flow into CloudTrail, which feeds GuardDuty for tamper detection. Third, guardrail interventions flow into CloudWatch metrics, which drive an alarm on InvocationsIntervened. Separately, data sources such as the S3 corpus feed into the Glue Data Catalog, which feeds DataZone for the lineage view. The teaching point the diagram embeds is the three-question split — content goes to Model Invocation Logging and S3/Athena, activity goes to CloudTrail and GuardDuty, behavior goes to CloudWatch alarms — with lineage handled by Glue and DataZone as a fourth, separate concern.
...
**A:** B — CloudTrail records that InvokeModel or Converse was called and by whom, but it does not capture the prompt or completion body (responseElements is null). The only service that logs the actual content of inferences is Amazon Bedrock Model Invocation Logging, and it is disabled by default — you must enable it via the console or PutModelInvocationLoggingConfiguration and provision an S3 bucket or CloudWatch log group first. A is wrong because data events still would not contain the content; C and D do not explain missing content.
...
Most "choose the right safety control" questions can be walked through the same two questions: what is the threat, and how complex is the flow. The threat dimension picks the policy type — harmful content maps to content filters, a whole subject area to denied topics, a specific word or name to a word filter, PII to either the sensitive information filter (inline) or Macie-plus-Comprehend (in the corpus), hallucination to contextual grounding backed by Knowledge Base retrieval, and prompt injection to the Prompt Attack content filter. The flow dimension decides the surrounding architecture — one uniform policy means Guardrails alone, a multi-stage or human-in-the-loop flow means Step Functions plus Lambda orchestration, and a model that is not on Bedrock means the ApplyGuardrail API. The last branch is placement: a cheap pre-check belongs in the input layer, while verifying the actual answer belongs in the output layer.

**Diagram (described):** This decision tree starts at "Safety control selection" and splits into two parallel questions. The first question is "What is the threat?" and branches by threat type: harmful content leads to content filters; a whole subject area leads to denied topics; a specific word or name leads to a word filter; PII exposure leads to a sub-question "Where is the PII?" which branches to the sensitive-info filter if inline at the model, or to "Macie then Comprehend" if in the S3 corpus; hallucination leads to "grounding plus RAG"; and prompt injection leads to the prompt-attack filter. The second question is "How complex is the flow?" and branches by flow type: one uniform policy leads to Guardrails alone; a multi-stage or human-in-the-loop flow leads to Step Functions plus Lambda; and a non-Bedrock model leads to the ApplyGuardrail API. Finally, the content-filter and denied-topic branches feed into a placement question "Which layer?" which branches to input filtering (reject early, cheap) or output verification (verify the answer). The tree's teaching point is that you pick the policy by threat, pick the architecture by flow complexity, and pick the layer by whether you are rejecting early or verifying the answer.
...
**A:** eight (veracity and robustness is one combined dimension); the prompt and response body (responseElements is null); disabled (off — you must enable it); free-form conversational chatbot question-answering (there is no grounding source to score against). Each of these four is a high-frequency distractor anchor.
