...
This is Guide 02 of the AIP-C01 series; it builds on Guide 01 and is followed by a guide on agents and orchestration.
...
Each section is textbook-depth prose with comparison tables and described diagrams, ending with an Exam-Relevant Distinctions checklist and a Knowledge Check quiz.
...
**Diagram (described):** The RAG pipeline is shown as two stacked groups. The first group, "Ingestion (ahead of time)," flows in a straight line: source documents lead to extracting text, which leads to chunking the text into passages, which feeds an embedding model, whose output is written into a vector index that also stores a source map. The second group, "Runtime (per query)," begins with a user query that is fed into the same embedding model, producing a query vector. That query vector flows into a similarity search step, which also draws on the vector index built during ingestion. The search returns retrieved chunks, which combine with the original user query to form an augmented prompt. That augmented prompt is sent to a generative model, which produces a grounded, cited answer. The key visual point is that one embedding model feeds both phases and the index built in ingestion is the thing the runtime search reads from.
...
**Diagram (described):** The data pipeline is shown as a left-to-right main flow with service callouts hanging off each stage. The main flow runs: raw mixed data leads to a validate-quality stage, then a process-multimodal stage, then an enrich-and-normalize stage, then a format-for-inference stage, and finally into chunking and embedding. Branching off the validate stage are three services: Glue Data Quality, Data Wrangler, and Lambda for custom checks. Branching off the process stage are three services: Transcribe for audio, Textract for scans, and SageMaker Processing. Branching off the enrich stage are two services: Comprehend for entities and Lambda for normalization. The diagram visually maps each pipeline phase to the AWS services that perform it.
...
**A:** 256, 512, and 1024 (default 1024); ~97% accuracy at ~75% storage savings. These numbers are the concrete justification for dropping to a smaller dimension when storage cost matters more than the last sliver of retrieval precision. Note this is the float32 dimension knob — distinct from switching to binary vectors, which is a separate, OpenSearch-only cost lever.

> **Source attribution:** Embedding concepts, float32 vs binary tradeoff, the OpenSearch-only binary constraint, and embedding/store coupling are MCP-researched from the Amazon Bedrock "How knowledge bases work" and "vector store prerequisites" documentation. Titan V2 dimensions (256/512/1024, default 1024), the 8,192-token input limit, the ~97% accuracy at ~75% storage figure, binary embeddingTypes support, and multimodal models are from the Titan Text Embeddings and Titan Multimodal Embeddings G1 documentation. Selection logic maps to Skill 1.5.2.
...
**Diagram (described):** This is a decision tree for choosing a chunking strategy from a source document. Starting at the source document, you evaluate the document's characteristics, which branches four ways. If the data is already pre-chunked, choose NONE (one chunk per file). If the content is unstructured with no clear topics, choose FIXED_SIZE with overlap. If the content has natural topic boundaries, choose SEMANTIC. If the document has a sectioned structure, choose HIERARCHICAL with its parent-and-child arrangement.
...
**A:** C — Hierarchical chunking creates smaller child chunks (for precise matching) derived from larger parent chunks (for surrounding context), which is exactly what structured, sectioned documents call for when you want both precision and context. NONE (A) would store whole files; fixed-size with no overlap (B) ignores the structure and risks cutting context; D is not a real strategy.
...
**A:** Switch to semantic chunking. Semantic chunking uses a foundation model to place boundaries where the topic actually shifts, so each chunk is a single coherent idea instead of a muddy mix. Fixed-size splitting at arbitrary token counts is what produced the multi-topic chunks; aligning boundaries to topics fixes the muddiness and improves retrieval relevance. The tradeoff is added ingestion cost from the foundation model.
...
**A:** False. The chunking strategy is set at data source creation and cannot be changed afterward. To move from fixed-size to semantic chunking you must create a new data source with the desired strategy and re-ingest the content. This immutability is why chunking choice deserves up-front thought rather than being treated as a tunable knob.

> **Source attribution:** The four chunking strategies (FIXED_SIZE, HIERARCHICAL, SEMANTIC, NONE), the default ~300-token sentence-aware behavior, and per-strategy parameters are MCP-researched from the Amazon Bedrock "How content chunking works" documentation and the ChunkingConfiguration, HierarchicalChunkingConfiguration, and SemanticChunkingConfiguration API references. The required overlapPercentage (1-99), hierarchical overlapTokens / 8,192-token levels / child-to-parent retrieval, semantic foundation-model cost, and the post-creation immutability of chunking strategy are verified from those sources; the custom transformation Lambda (POST_CHUNKING) is from the Bedrock custom transformation documentation. Selection and troubleshooting guidance map to Skills 1.5.1 and 5.2.1.
...
**Diagram (described):** This is a decision tree for choosing a vector store. Starting from "choose a vector store," the first decision is whether binary vectors are needed. If yes, the answer is either OpenSearch option (Serverless or Managed). If no, you proceed to a second decision about the primary need, which branches five ways: lowest cost for infrequent queries leads to S3 Vectors at sub-second latency; multi-hop reasoning leads to Neptune Analytics GraphRAG; SQL plus vectors together leads to Aurora PostgreSQL pgvector; low latency at large scale leads to OpenSearch Serverless; and an existing third-party investment leads to Pinecone, Redis, or MongoDB Atlas.
...
**A:** False. Quick-create is available only for OpenSearch Serverless, Aurora PostgreSQL Serverless, Neptune Analytics, and S3 Vectors. OpenSearch Managed clusters and all three third-party stores (Pinecone, Redis Enterprise Cloud, MongoDB Atlas) are bring-your-own — you must create and configure the index yourself before creating the knowledge base, then give Bedrock the field names.
...
**Diagram (described):** This is a retrieval funnel flowing top to bottom. A user query first reaches a transform-query decision: if the query is complex and multi-part, it is decomposed into sub-queries; if simple, it passes straight through. Either path then arrives at a search-type decision that branches two ways — SEMANTIC does vector-similarity-only search, while HYBRID does vector plus keyword search. Both search paths then converge on an apply-metadata-filter step, which feeds into a step that limits results to numberOfResults, which feeds into a rerank-top-results step, and finally produces the augmented prompt context handed to the model. The funnel visually orders the knobs: transform, then search type, then filter, then result limit, then rerank.
...
**A:** 5 (the default `numberOfResults`), and the actual number can be *lower* than requested. `numberOfResults` is a maximum ceiling, not a guarantee. With hierarchical chunking, `numberOfResults` counts the child chunks matched, but children that share the same parent are replaced by that single parent chunk before results are returned — so multiple matched children collapse into one parent, shrinking the final count. Setting the value too low risks missing the needed chunk; too high floods the context window with marginal chunks, which is the dilution that reranking helps counter.
...
**Diagram (described):** This is a flow showing how an application query is routed. An application query reaches a "which API" decision that branches two ways: if you need a finished answer, you call RetrieveAndGenerate; if you need raw chunks, you call Retrieve. Both branches then converge on a shared step that embeds the query and searches the store, which produces the top relevant chunks. From there the two paths diverge by purpose: the RetrieveAndGenerate path takes the chunks into an augment-prompt-and-generate step that produces a grounded answer with citations, while the Retrieve path hands the same chunks to custom downstream processing. A side note attached to RetrieveAndGenerate shows that passing a sessionId enables built-in session memory.
...
Pattern A is event-driven, near-real-time sync, and it is a published AWS reference architecture rather than something you assemble from scratch. When an object lands in or changes in S3, an S3 event notification fires; EventBridge routes it to a Lambda event processor, which writes an audit record to DynamoDB and enqueues the work to an SQS queue. SQS exists here for a specific reason — to buffer bursts so the downstream does not exceed Bedrock's ingestion rate limits. A Step Functions state machine then orchestrates the actual sync: it checks the service quota, and if capacity is available it calls StartIngestionJob, otherwise it waits 300 seconds and rechecks before proceeding; once a job is running it monitors the job to completion. CloudWatch metrics and SNS alerts close the loop with observability. The reason this needs a state machine rather than a single Lambda calling StartIngestionJob directly is the job math: each StartIngestionJob syncs the data source as a unit (incrementally), concurrent ingestion jobs per knowledge base are capped by an adjustable quota (50 by default per the current quota table), and the control-plane API throttles call bursts. A naive "event triggers sync" design fires redundant per-file jobs straight into those ceilings under load; the buffer-and-orchestrate pattern is what makes event-driven sync survivable *(quota figures are point-in-time)*.

**Diagram (described):** This is a left-to-right event-driven sync architecture. An S3 data change emits an S3 event notification, which flows to EventBridge, which invokes a Lambda event processor. That Lambda does two things: it writes an audit record to a DynamoDB log, and it enqueues work into an SQS buffer queue. The SQS queue feeds a Step Functions state machine, which reaches a "quota available" decision. If quota is available, it calls StartIngestionJob; if not, it waits 300 seconds and loops back to recheck the quota. Once StartIngestionJob runs, it updates the Bedrock Knowledge Base, and the knowledge base activity is observed through CloudWatch and SNS alerts. The diagram emphasizes the SQS buffer and the Step Functions quota check as the elements that protect against Bedrock's ingestion limits.
...
**Diagram (described):** This is a small decision tree comparing the two freshness patterns. Starting from a "freshness latency" question, there are two branches. If the index must be current within minutes after a change, choose the event-driven sync (Pattern A), which works by having S3 events trigger the sync. If a fixed cadence is acceptable, choose scheduled refresh (Pattern B), which works by having a scheduler trigger the sync.
...
- Ingestion quotas *(point-in-time)*: 50 concurrent ingestion jobs per knowledge base (adjustable), 200 data sources per KB; StartIngestionJob call bursts are control-plane throttled — batch events rather than firing one job per file.
...
**Diagram (described):** This is the master four-stage RAG design decision cascade. Stage one starts at "design RAG on AWS" and asks whether to build the plumbing yourself: the default answer "no" leads to Bedrock Knowledge Bases, while "yes, special need" leads to a custom RAG pipeline. Both of those converge into stage two, the vector-store-driver decision, which branches five ways: binary vectors lead to either OpenSearch option; lowest cost for cold data leads to S3 Vectors; multi-hop reasoning leads to Neptune Analytics; SQL plus vectors leads to Aurora pgvector; and low latency at scale leads to OpenSearch Serverless. All five store choices converge into stage three, the chunking-driver decision, which branches four ways: pre-chunked files lead to NONE; topic shifts lead to SEMANTIC; structured sections lead to HIERARCHICAL; and plain prose default leads to FIXED_SIZE with overlap. All four chunking choices converge into stage four, the retrieval-symptom decision, which branches four ways: exact tokens missed leads to HYBRID search; the best chunk ranked low leads to adding reranking; a multi-part question leads to query decomposition; and a need to scope by attribute leads to metadata filtering. The diagram's whole point is that the four stages are walked in this fixed order.
...
**A:** four (OpenSearch Serverless, Aurora PostgreSQL Serverless, Neptune Analytics, S3 Vectors); 5; maximum (ceiling); OpenSearch Serverless and OpenSearch Managed. These four numbers and constraints are among the most directly tested facts in the guide — quick-create scope, the default result ceiling, and the OpenSearch-only binary capability each anchor a recurring distractor.
...
**A:** They applied the unstructured (vector) playbook to a structured data store. A Redshift-backed structured store uses no embeddings and no vector index at all — Bedrock generates SQL from the natural-language question via GenerateQuery and runs it against Redshift as the query engine. Because there is no vector search, numberOfResults, search type (semantic vs hybrid), and metadata filtering simply do not apply, which is why tuning them has no effect. The correct configuration is a structured store with a query engine (Redshift Provisioned or Serverless), a storage configuration (Redshift databases or a Glue Data Catalog), and optional query generation tuning such as table and column descriptions and curated queries to improve the generated SQL.
...
**Q1:** A user-facing assistant runs on a Bedrock knowledge base. Users mix conceptual questions ("how do refunds work?") with exact tokens (SKUs, error codes like ThrottlingException). The team sets overrideSearchType to HYBRID and sees no errors, yet exact-token matches still miss. Which TWO statements explain the situation or fix it? (Select TWO)
...
- C) Automate ongoing freshness with an event-driven sync (S3 to EventBridge to Lambda to StartIngestionJob) or a scheduled EventBridge Scheduler job
